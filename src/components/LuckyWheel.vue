...
export default {
  ...
  // Add the new prize to topWinnings
  const updatedWinnings = [...this.topWinnings, newPrize];

  // Filter out duplicates by creating a Map where the value is the key
  const uniqueWinnings = Array.from(
    new Map(updatedWinnings.map(item => [item.value, item])).values()
  );

  // Sort the winnings by value in descending order
  uniqueWinnings.sort((a, b) => b.value - a.value);

  // Store only the top 3 winnings
  this.topWinnings = uniqueWinnings.slice(0, 3);
